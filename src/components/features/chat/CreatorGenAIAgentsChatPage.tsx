...
export interface AgentMeta { id: GenAIAgentType; title: string; subtitle: string; icon: string; color: string; bgColor: string; borderColor: string; capabilities: string[]; quickPrompts: { label: string; prompt: string }[]; }
...
export const CreatorGenAIAgentsChatPage: React.FC<CreatorGenAIAgentsChatPageProps> = ({ user, userName = "Creator", products = [], userLocation, onRequestLocationPermission }) => {
  ...
  const handleRunTemplateGeneration = async (tmpl?: CreativeTemplate) => {
    ...
    try {
      ...
      // Propagate or handle the actual backend error instead of synthetic success
      throw err;
    } finally { 
