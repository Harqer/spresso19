...
export const GenkitCreativeStudioModal: React.FC<GenkitCreativeStudioModalProps> = ({ product, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/70 p-4 backdrop-blur-md">
      <div className="my-auto flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl border border-[#d8ebd7] bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-[#d8ebd7] bg-[#f2f8f2] p-6">
          <div>
            <h2 className="text-lg font-bold text-[#18211e]">Creative campaign ideas</h2>
            <p className="mt-1 text-xs text-[#5e635f]">{product.brand} · {product.name}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
