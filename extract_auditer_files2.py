...
for line in lines:
    ...
    if in_code:
        if clean_line.startswith("package components.features."):
            ...
            if clean_line.endswith(".kt") and " " not in clean_line:
                # We hit the next file or a new section
                ...
                in_code = False
                code_lines = []
            elif "Here is the audit" in clean_line or clean_line == "Refactored Source Files" or clean_line == "Code Audit Findings" or "The above content" in clean_line:
                # End of code block
                code_str = "".join(code_lines)
                pkg_match = re.search(r'package\s+([A-Za-z0-9_.]+)', code_str)
                if pkg_match:
