...
- **Data Type**: MUSIC
  *Description: A user’s music files.*
  - `server/geminiService.ts (Pattern: track)`
- **Data Type**: OTHER_APP_PERFORMANCE
  *Description: Any other app performance data excluding crash log data (e.g., stack traces) and diagnostics (e.g., battery life, loading time, latency, or framerate).*
  - `dist/assets/index-BunGqtNk.js (Pattern: heartbeat)`
- `server/inventory.ts (Pattern: telemetry)`
- `src/db/index.ts (Pattern: telemetry)`
- **Data Type**: PRECISE_LOCATION
  *Description: User or device physical location within an area less than 3 square kilometers, such as location provided by Android’s ACCESS_FINE_LOCATION permission.*
  - `dist/assets/index-BunGqtNk.js (Pattern: latitude)`
- `server/geminiService.ts (Pattern: latitude)`

**UI Disclosure Starting Points**:
- `composeApp/src/androidMain/kotlin/com/spresso19/MainActivity.kt (Pattern: share)`
- `composeApp/src/androidMain/kotlin/ui/ImagePicker.kt (Pattern: AlertDialog)`
- `dist/assets/index-BunGqtNk.js (Pattern: Dialog)`
- `functions/lib/index.js (Pattern: Accept)`
- `functions/src/index.ts (Pattern: Accept)`
- `server/routes.ts (Pattern: collect)`
- `src/components/ElevatedQuickActionFab.tsx (Pattern: share)`
- `src/components/MaterialIcon.tsx (Pattern: share)`
- `src/components/OrdersTracker.tsx (Pattern: collect)`
- `src/components/ProductDetailsModal.tsx (Pattern: share)`
- `src/components/SeasonalCategorySection.tsx (Pattern: collect)`
- `src/components/pages/WardrobeViewPage.tsx (Pattern: collect)`
- `src/lib/firebase.ts (Pattern: collect)`
- `src/lib/location.ts (Pattern: Accept)`

## Output schema

Save final JSON output to `/home/shaolin/Spresso/spresso19/.scratch/play_policy_insights_69e7f064-aa0f-456e-9d99-cfbd0d13817b/worker_data_safety_part_5.json`.
