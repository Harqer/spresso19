...
- **Checkout authorization boundary (updated 2026-09-22)**: Spresso-controlled Stripe checkout and merchant-browser checkout preparation are separate paths, but both require the same human-controlled financial boundary. The server verifies the current merchant offer and binds authorization to the exact immutable intent (user, merchant, listing/variant, quantity, amount, currency, payment method where applicable, nonce, expiry, idempotency). The final charge/order commit requires server-verifiable biometric/passkey + MFA proof; a client-only biometric result or an authenticated Firebase session is not sufficient. The current saved-card/off-session implementation must not be treated as complete until that proof is verified server-side. Signed provider/webhook reconciliation remains authoritative for payment outcome.
- **Agentic browsing**: Merchant automation uses Convex-owned typed tools/state with Cloudflare Browser Sessions. Kitesurf is for compatible short/stateless tasks; Chromium Browser Run is used for authenticated/persistent/HITL flows. Sessions require HTTPS + destination guardrails, SSRF protection, bounded actions, secret/PII redaction, prompt-injection-resistant schemas, idempotency/reconciliation, and auditable semantic events. Agents may automate reversible merchant-cart actions. Account creation/profile disclosure needs explicit approval; MFA/SSO/CAPTCHA/sensitive credential entry uses Human in the Loop; final purchase commit remains the separate exact-transaction authorization boundary. Never give the LLM unrestricted Playwright/CDP/JavaScript access.
